...
### Q: How did you handle double-booking prevention in a distributed environment?
> **Answer**: "I implemented a two-tier concurrency safety mechanism. First, a distributed Redis `SETNX` lock keyed by item ID serializes incoming booking attempts at the application layer. Second, within a database transaction (`$transaction`), I perform a strict date range overlap query on confirmed booking records before creating the reservation. To prove its resilience, I authored a load test using 100 simultaneous concurrent requests for the exact same item and slot. Under load, exactly 1 request succeeded with an HTTP 201, while 99 were rejected with an HTTP 409 Conflict with zero double-bookings."

### Q: Why compile contract snapshots on the agreement instead of referencing live templates?
> **Answer**: "Referencing live templates creates legal vulnerability — if an owner alters clause parameters after a booking is requested, active rentals would retroactively inherit modified terms. To enforce contract immutability, the contract engine compiles and freezes a JSON snapshot (`compiledClauses`) on the Agreement at the moment of acceptance. Even if the owner modifies or deletes the template later, the agreement's compiled clauses remain completely unchanged."

### Q: How does the trust score engine differ from a simple review average?
> **Answer**: "Simple average ratings are vulnerable to manipulation and ignore recency. RentAny implements a recomputable, explainable trust algorithm that calculates a normalized score from 0.0 to 100.0. It incorporates an exponential time decay factor ($1 / (1 + \text{days}/30)$), weights reviews by the reviewer's own trust score, rewards transaction volume (+2.5 pts per completion), and penalizes adverse dispute outcomes (-15 pts per lost dispute)."
